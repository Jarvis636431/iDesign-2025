// 展厅基础信息配置，可全局复用
export const halls = [
  {
    id: 73,
    name: '启风山谷',
    enName: 'First Whisper',
    subTitle: '风起于谷，思由此生。',
    enSubTitle: "Wind's valley sigh, lets ideas fly.",
    desc: '这里是设计之风初次掠过的山谷，灵感如晨雾升腾，念头如嫩芽破土。每一道线条，都是风中醒来的低语。',
    enDesc: 'This is the valley first brushed by the design breeze, Where inspiration ascends like morning mist, and notions break forth like tender shoots. Every line drawn is a whisper awakened in the wind.',
    color: '#2FA3B0',
    backgroundColor: '#D5f7ec',
    icon: import.meta.env.BASE_URL + 'assets/images/halls/hall-icons/icon73.png',
    logo: import.meta.env.BASE_URL + 'assets/images/halls/hall-logos/hall-73.png',
    border: import.meta.env.BASE_URL + 'assets/images/halls/hall-borders/border73.png',
  },
  {
    id: 74,
    name: '穿林探径',
    enName: 'Winding Trails',
    subTitle: '风入林深，路在枝间。',
    enSubTitle: 'Wind-woven ways, through leafy maze.',
    desc: '方法如林，步步需辨。你将看到思维如何穿梭幽径，在交错的枝条间，试探出属于自己的路径。',
    enDesc: 'Methods, like a forest, demand discernment at every step. Here, you will witness how thought navigates hidden trails, Testing and forging its own way among the interwoven boughs.',
    color: '#6CB130',
    backgroundColor: '#e6f2c6',
    icon: import.meta.env.BASE_URL + 'assets/images/halls/hall-icons/icon74.png',
    logo: import.meta.env.BASE_URL + 'assets/images/halls/hall-logos/hall-74.png',
    border: import.meta.env.BASE_URL + 'assets/images/halls/hall-borders/border74.png',
  },
  {
    id: 75,
    name: '驰风原野',
    enName: 'Unveiled Horizons',
    subTitle: '风驰其野，万象铺展。',
    enSubTitle: 'Wind-swept plains, where vision reigns.',
    desc: '在这片开阔的原野上，设计不再犹疑，它奔涌、它落地、它成形。这是语言与结构的自由之地，也是愿景之所托。',
    enDesc: 'Upon this open expanse, Design hesitates no more; it surges, it lands, it takes shape. This is a realm of freedom for language and structure, a vessel for envisioned futures.',
    color: '#F6c345',
    backgroundColor: '#fefbe6',
    icon: import.meta.env.BASE_URL + 'assets/images/halls/hall-icons/icon75.png',
    logo: import.meta.env.BASE_URL + 'assets/images/halls/hall-logos/hall-75.png',
    border: import.meta.env.BASE_URL + 'assets/images/halls/hall-borders/border75.png',
  },
  {
    id: 76,
    name: '掠泽听声',
    enName: 'Resonant Ripples',
    subTitle: '风掠泽面，水光生响。',
    enSubTitle: 'Wind-stirred lakes, an echo makes.',
    desc: '风不止于吹动，亦激起回响。在这里，你将听见合作的音节，社会的涟漪，与设计者投向世界的一声温柔问候。',
    enDesc: 'The wind does more than merely blow; it also stirs echoes. Here, you will hear the syllables of collaboration, the ripples of society, And a gentle greeting cast by designers unto the world.',
    color: '#4950A0',
    backgroundColor: '#e2daf3',
    icon: import.meta.env.BASE_URL + 'assets/images/halls/hall-icons/icon76.png',
    logo: import.meta.env.BASE_URL + 'assets/images/halls/hall-logos/hall-76.png',
    border: import.meta.env.BASE_URL + 'assets/images/halls/hall-borders/border76.png',
  },
  {
    id: 77,
    name: '终风化木',
    enName: 'Crowned Woods',
    subTitle: '风归林深，群木成森。',
    enSubTitle: "Wind's haven found, a forest crowned.",
    desc: '这是一场回归，也是一场出发。风在此停驻，设计在此归聚。一代人的思索与创造，于林中静静生长。',
    enDesc: 'This is a return, and also a new departure. The wind settles here; design converges herein. The reflections and creations of a generation quietly grow within the woods.',
    color: '#E77E37',
    backgroundColor: '#ffe9d5',
    icon: import.meta.env.BASE_URL + 'assets/images/halls/hall-icons/icon77.png',
    logo: import.meta.env.BASE_URL + 'assets/images/halls/hall-logos/hall-77.png',
    border: import.meta.env.BASE_URL + 'assets/images/halls/hall-borders/border77.png',
  }
]
